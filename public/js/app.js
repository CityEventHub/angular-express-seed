...
,function($routeProvider,  $locationProvider,  $provide) {
	...
	$provide.decorator('$resource',['$delegate',function($delegate) {
		return function addUpdateMethod(url, paramDefaults, actions) {
			...
			var unParam = angular.copy(paramDefaults);
			for (var param in unParam) {
				if(unParam.hasOwnProperty(param)) {
					unParam[param] = "";
				}
			}
			actions = angular.extend({}, {update: {method: "PUT"},
										  save:   {method: "POST", params: unParam}}, actions);
